...
parser.add_argument('--GRAYAUG', default=1, type=int)
...
def samplewise_perturbation_eval(random_noise, data_loader, model, eval_target='train_dataset', mask_cord_list=[], GRAYAUG=False):
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        ...
        if random_noise is not None:
            for i, (image, label) in enumerate(zip(images, labels)):
                ...
                if GRAYAUG:
                    images[i] = transforms.Grayscale(3)(images[i]) + sample_noise
                else:
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, GRAYAUG=False):

    if GRAYAUG:
        no_train_augments_var=False
    else:
        no_train_augments_var=True

    print('No train augmentation during poisoning:',no_train_augments_var)
    ...
    datasets_generator = dataset.DatasetGenerator(train_batch_size=args.train_batch_size,
                                                  ...
                                                  seed=args.seed, no_train_augments=no_train_augments_var)
    ...
    while condition:
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader['train_dataset']), total=len(data_loader['train_dataset'])):
            ...
            if args.attack_type == 'min-min':
                perturb_img, eta = noise_generator.min_min_attack(images, labels, model, optimizer, criterion, random_noise=batch_noise, GRAYAUG=GRAYAUG)
            else:
...
def main():
    ...
    if args.attack_type == 'min-min':
        ...
        if args.perturb_type == 'samplewise':
            noise = sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, args.GRAYAUG)
        torch.save(noise, os.path.join(args.exp_name + '/' + args.version, 'perturbation.pt'))
        logger.info(noise)
        logger.info(noise.shape)
        logger.info('Noise saved at %s' % (os.path.join(args.exp_name + '/' + args.version, 'perturbation.pt')))
    else:
